...
namespace RegulatedNoise.Trading
{
    public class TradeEngine
    {
        ...
        public static Tuple<IEnumerable<TradeRoute>, IEnumerable<TradeRoute>> GetBestRoundTripBetweenTwoStations(string stationFrom, string stationTo, out int bestRoundTrip)
        {
            ...
            int outwardIncome = 0;
            int returnIncome = 0;
            IEnumerable<MarketDataRow> toStationMarket = ApplicationContext.Commodities.StationMarket(stationTo);

            foreach (var fromRow in ApplicationContext.Commodities.StationMarket(stationFrom))
            {
