        ...
        public EDDN.OutfittingObject GetOutfittingFromCompanion(Newtonsoft.Json.Linq.JToken outfittingItem, Boolean entitled = false)
        {
            ...
            try
            {
                ...
                if(outfitting != null)
                {
                    ...
                    // Entitlements
                    if(!String.IsNullOrEmpty((String)(outfittingItem.SelectToken("sku", false) ?? "")))
                    {
                        String sku = outfittingItem.SelectToken("sku").ToString();

                        if(sku.StartsWith("ELITE_SPECIFIC_V_POWER"))
                        {     
                            outfitting.Entitlement ="powerplay";
                        }
                        else if(sku != "ELITE_HORIZONS_V_PLANETARY_LANDINGS")
                        {
                            throw new NotSupportedException(String.Format("{0}: Unknown sku {1}", outfittingItem.SelectToken("id"), sku));
                        }
