...
namespace IBE.Ocr
{
    public partial class tabOCR : UserControl
    {
        public const String        DB_GROUPNAME                    = "OCR";
        private Ocr                 m_DataSource;                   // data object
        ...
        private void Acquisition(bool noAutoImport = false)
        {
            ...
            if (_screenshotResultsBuffer.Count == 0)
            {
                ...
                if (_preOcrBuffer.Count == 0 && ocr.ScreenshotBuffer.Count == 0)
                {
                    ...
                    if (Program.PlausibiltyCheck.CheckPricePlausibility(tbFinalOcrOutput.Text.Replace("\r", "").Split('\n')))
                    {
